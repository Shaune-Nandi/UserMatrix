...
@section('title', 'create persmission')

@section('body')
    <div class="grid">
        <form action="/permissions/create" method="POST" class="justify-self-center bg-gray-100 rounded-md w-2/3 mt-5 mx-5 px-5 py-5">
            @csrf
            <h3 class="text-2xl ml-10 mb-6">Create new permission</h3>
            <div class="relative z-0 mb-6 w-full group">
                <input type="text" name="permission_name" id="permission_name" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required="">
                <label for="permission_name" class="absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Permission Name</label>
            </div>
