...
@section('body')
    ...
    <div class="grid">
        <form action="/user/create" method="POST" class="justify-self-center bg-gray-100 rounded-md w-2/3 mt-5 mx-5 px-5 py-5">
            @csrf
            <h3 class="text-2xl ml-10 mb-6">Create user</h3>
            ...
            <fieldset class="mt-10">
                <legend class="text-sm font-semibold mb-1">Select role:</legend>
                @foreach($role as $role)
                    ...
                    <div class="flex items-center ml-10 mb-1">
                        ...
                        <label for="role[]" class="ml-3 text-sm font-sm text-gray-900 dark:text-gray-300">{{ $role->name }} ({{ $role->slug }})</label>
                    </div>
                
                @endforeach
            </fieldset>

                <input hidden value="12345" type="password" name="password" id="password">
